...
ta_sum_age <- res_age %>% group_by(tax, cr, age, sites) %>% 
          summarize(ta_in_ci = mean(age_est_lb < age & age_est_ub > age),
                    ta_rmse = sqrt(mean((age_est - age)^2)),
...
plt_cr_bf <- ggplot(bf_sum_cr) +
        ...
        theme_minimal() + 
        labs(x = "Sites", y = "Rate of preference for clock model") + 
        scale_color_grey()
...
### AGE
## plot 1: mse
plt_age_rmse <- ggplot(ta_sum_age) +
        geom_point(aes(x=factor(sites),
                       y=ta_rmse,
                       col=factor(tax),
                       shape=interaction(cr,age))) +
...
plt_age_cov <- ggplot(ta_sum_age) +
        ...
        labs(x = "Sites", y = "MRCA Age CI Coverage") + 
        scale_color_grey()
...
plt_age_tr_topo <- ggplot(tree_sum_age) +
        ...
        geom_line(aes(x=factor(sites),
                       ...
                       group=interaction(tax,cr,age))) + 
        theme_minimal() + 
        labs(x = "Sites", y = "True Tree Coverage") + 
        scale_color_grey()
...
top <- ggarrange(
        ...
        plt_cr_ci_wid, 
        plt_tr_topo_cr,
        plt_cr_bf, ncol=5, common.legend=TRUE)
bottom <- ggarrange(
        plt_age_rmse,
        plt_age_cov,
        plt_age_ci_wid,
...
plt_sim2 <- ggarrange(
    plt_snp_cv,
    plt_snp_ci_wid,
    plt_snp_tr_topo, ncol=3, common.legend=TRUE)

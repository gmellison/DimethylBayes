...
for (clone in clones) {
    ...
    for (clock in c("","_rc","_sc")) {
        for (partition in c(1, 5, 10)) {
            ...
            fc <- file(sprintf("%s/%s", out_dir, nex_file), "w")
            ...
            mod_mcmc_lines <- c("begin mrbayes;",
               ...
               ifelse(subset_data,sprintf("exclude %s-%s;",sites_include+1, charset_idx-1), ""),
               partition_set,
               clock_rate_lines,
               tree_age_lines,
               "lset rates=gamma;",
            ...
            sum_file <- sprintf("%s/sum_%s_%s_%s.nex", out_dir, clone, partition, clock)
            ...
            c("begin mrbayes;",
               sprintf("log start filename=%s;",log_file),
               sprintf("sump burninfrac=%s filename=%s;", burnin/nreps, nex_file),
               sprintf("sumt burninfrac=%s filename=%s;", burnin/nreps, nex_file),
               sprintf("log stop filename=%s;",log_file),
               "end;"))
            close(fc)
...
system(sprintf("scp -r %s/*.nex gme88782@xfer.gacrc.uga.edu:/scratch/gme88782/dim/", out_dir))

## submit
#cmd <- sprintf("ssh gme88782@sapelo2.gacrc.uga.edu \'cd /scratch/gme88782/%s/ ; sbatch %s \'", cluster_dir, "sub_all.sh")
#system(cmd)
